...
struct CodexQuotaRings: View {
    ...
    var body: some View {
        ZStack {
            ...
            case .longAndShort:
                ...
                arc(progress: weekly ?? 0, start: 60, sweep: 60, width: 2.7)
                    .opacity(0.72)
            case .leftAndRight:
                arc(progress: fiveHour, start: 102, sweep: 156, width: 2.7)
                arc(progress: weekly ?? 0, start: -78, sweep: 156, width: 2.7)
                    .opacity(0.72)
            }
        }
    ...
    private func arc(progress: Double, start: Double, sweep: Double, width: CGFloat) -> some View {
        ...
        return ZStack {
            ...
            if fraction > 0 {
                Circle().trim(from: 0, to: sweep / 360 * fraction)
