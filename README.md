...
- 悬停预览使用无回弹的平滑展开曲线，并围绕实体刘海对称扩展；下层由左右状态与中央通知/展开入口组成，不留下空白中段。
- 完整面板展开后，点击面板外部、桌面或其他 App 会自动收起；面板内菜单和确认弹窗不受影响。
- 默认左翼为正在播放、右翼为 Codex；媒体翼只在实际播放时出现，其他配置保持常驻。
- 展开面板顶部的滑杆菜单可分别配置左翼和右翼：电池状态、ChatGPT / Codex 额度、正在播放或隐藏，并支持左右互换；选择会跨启动保存。
- 电池圆环中央在普通状态显示电池图标，充电时显示实时电池充电瓦数；额度圆环中央始终显示剩余百分比。
...
- “更多”菜单提供通知设置、动画预览、辅助功能授权和明确的退出入口。
...
7. 使用“更多 → 预览通知动画”先检查图标动画，再用真实通知验证系统横幅与通知中心行为。
8. 使用“更多 → 退出 Notch Triage”正常结束应用。
9. 展开后默认进入“电源”页；切换 80/85/90/95/100% 会立即写入系统充电上限，“充满”只临时覆盖限制。
10. 点击标题栏的滑杆图标，分别更改左右显示内容；收起面板检查圆环，再重启 App 验证配置是否保留。
